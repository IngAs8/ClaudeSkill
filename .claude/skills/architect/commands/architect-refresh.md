...
The research contract lives in `agents/stack-researcher.md`. Read it first.
...
4. Re-run `blueprint-validator` (`agents/blueprint-validator.md`). A refresh
...
1. **Pins live in exactly one layer of the knowledge base.** The `stack-researcher` report from this
   session is the authority; `knowledge/runtime-tracks/` is a cache that drifts
