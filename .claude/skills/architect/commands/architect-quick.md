...
- The `architect` skill: `SKILL.md`, in the skill's own directory (the folder this `commands/`
  directory sits in). Invoke it with the `Skill` tool (`skill: architect`) if it has not
  already loaded this turn, then read it end to end.
- Every bare path below (`knowledge/…`, `questions/…`, `templates/…`, `agents/…`) resolves
  against that same skill directory. Only `./blueprints/` is cwd-relative.
...
| Output | **Recommend single file** — quick mode implies a small build — and confirm it, because `questions/phase-4-generate.md` **Step 2 is authoritative on emission mode**. This row is input to your recommendation, not the decision. Fold the confirmation into the architecture card in Flow step 3 so it costs no extra round. Confirmed single file → `./blueprints/<project-slug>-blueprint.md` in the user's cwd; bundle → `./blueprints/<project-slug>/`. |
...
| Decision | Default |
|---|---|
| Shape | Classify from the three answers using `questions/phase-1-discovery.md`. Ambiguous → pick the closer one and say which and why. |
...
| Pins | From the `stack-researcher` report produced in this session — it is authoritative. `knowledge/runtime-tracks/<track>.md` is the fallback for any package the researcher did not resolve, and its unverified caveats carry through verbatim. Never write a pin from memory. |
...
2. Classify the shape. Read `knowledge/shapes/<shape>.md` and its default
   runtime track. Check `knowledge/stack-compatibility.md` for known-bad pairs.
...
5. Dispatch `blueprint-writer` to emit the blueprint from
   ...
   `templates/tasks-schema.md`, and
   `templates/epic-template.md`. Tell it the output mode explicitly — the one
