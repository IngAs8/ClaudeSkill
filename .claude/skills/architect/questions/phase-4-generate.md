...
the plugin root — open it as `<path>`. The one exception is `./blueprints/`,
...
**Write to the user's current working directory. Never inside the skill directory.**

The skill directory is shared across every project and may be replaced wholesale on update.
Anything written there is lost and unreachable from the user's project.

- ✅ `./blueprints/<project-slug>/blueprint.md`
- ❌ `<skill-dir>/blueprints/...`
