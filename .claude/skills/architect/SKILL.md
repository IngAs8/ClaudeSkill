...
10. **Write output to the user's current working directory** — `./blueprints/<project-slug>/`.
    Never write inside the skill's own directory; it is shared across every project and is not a
    writable workspace.
...
| GENERATE | gate passed | `questions/phase-4-generate.md` | Files written, validator clean |
...
**Path resolution.** Every bare path in this file and inside `questions/`, `templates/`,
`agents/` and `knowledge/` files is relative to **this skill's own directory** — the folder that
contains this `SKILL.md`. Resolve `knowledge/shapes/saas-webapp.md` against that folder, not
against the user's project. The one exception is `./blueprints/`, which is always the **user's
...
- Pick the **runtime track** — read it from `knowledge/runtime-tracks/`.
...
- Pick each **capability** — read the relevant files from
  `knowledge/capabilities/` (auth, database, deployment, payments-rails,
...
1. **Read `questions/phase-4-generate.md` and execute it in order** — the
...
3. **Present nothing until the validation passes.** Send `blueprint-validator`'s findings back to the
   ...
   `agents/blueprint-validator.md` and say the audit was self-run. The bar
...
**Read `commands/architect-brownfield.md` and follow it end to end** —
...
you to touch**, and the repo's existing conventions beat this skill's defaults.
...
delegation is not. `questions/phase-4-generate.md`, *Never hard-depend on a
...
slash is a silent no-op.** Full table with fallbacks: `knowledge/skills-registry.md`.
...
- `agents/blueprint-writer.md` — what the writer expects in its brief
