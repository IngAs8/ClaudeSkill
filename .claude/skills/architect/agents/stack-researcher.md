...
`knowledge/runtime-tracks/<track>.md` is a **cache, not a source of truth** —
...
8. **Never edit files.** Even if you notice a wrong pin in
   `knowledge/runtime-tracks/*.md` — report it, do not fix it.
...
Use `Read`/`Grep` on `knowledge/runtime-tracks/` to see what is currently
...
- `knowledge/runtime-tracks/ts-node.md` — the only place pins are allowed to live
- `knowledge/stack-compatibility.md` — known-bad combinations, checked after versions resolve
- `agents/blueprint-writer.md` — the consumer of this report
- `commands/architect-refresh.md` — the refresh workflow that drives this agent
