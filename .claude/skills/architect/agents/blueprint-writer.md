...
| **Shape** (one of `knowledge/shapes/*.md`) | Build order skeleton, data model, directory structure |
| **Runtime track** (one of `knowledge/runtime-tracks/*.md`) | Fallback pins, setup commands, test/lint/build commands |
| **Capabilities** (from `knowledge/capabilities/*.md`) | Extra build steps, extra tables, extra env vars |
...
1. **Read the template first.** `templates/blueprint-template.md`. Its section
...
5. **Check `knowledge/stack-compatibility.md`** before writing the stack table.
...
6. **Read `knowledge/skills-registry.md`** for the skills section. Copy names
   and install commands verbatim. Never invent either.
7. **Read `templates/claude-md-template.md`** and produce the target project's
...
Everything below is relative to the **user's** current working directory — never inside the
`architect` skill directory. That directory is shared across projects and may be replaced on update. The slug is lowercase kebab-case
...
`tasks.json` per `templates/tasks-schema.md` (it is **JSON** — a bare array,
no wrapper — and it is the file `/architect-next` globs; a `tasks.md` is unreadable by every
consumer in this repo), epics per `templates/epic-template.md`, and the
...
8. **Never write outside `./blueprints/`.** `./blueprints/<slug>/` in bundle mode,
   `./blueprints/<slug>-blueprint.md` in single-file mode. Do not touch the skill directory or the
   user's source tree, and never write a `CLAUDE.md` anywhere but under the bundle's `workspace/`.
   **Every knowledge path you open is rooted at the `architect` skill directory; every path you
   write is cwd-rooted under `./blueprints/`.** Confusing the two is how a subagent reads nothing
   and writes into the skill directory.
...
- `templates/blueprint-template.md` — the section contract you fill
- `templates/claude-md-template.md` — the target project's CLAUDE.md (blueprint §19.1, under 200 lines)
- `templates/tasks-schema.md` — `tasks.json`, bundle mode only
- `templates/epic-template.md` — `epics/NN-<name>.md`, bundle mode only
- `knowledge/skills-registry.md` — verbatim skill names and install commands
- `knowledge/stack-compatibility.md` — check before writing the stack table
- `agents/stack-researcher.md` — where your pins come from
- `agents/blueprint-validator.md` — audits what you write; read its fail list before you write
