...
`Checkpoint` can produce.** Every step template in this skill orders **Do → Done when → Verify →
...
`Grep` `templates/blueprint-template.md` with the same pattern.
...
Its required parts are set by `commands/architect-brownfield.md`, which is the
...
against `knowledge/skills-registry.md` — it is authoritative.
...
`templates/blueprint-template.md` for the §11 table headers and see which
columns exist.
...
`templates/tasks-schema.md` — it is authoritative and you apply it as written,
...
assert anything the commit is what creates.** Every step template in this skill — `templates/blueprint-template.md` §9, `templates/epic-template.md`, the `verify` array in
...
- `agents/blueprint-writer.md` — writes what you audit; its hard rules are your fail list
- `agents/stack-researcher.md` — the authoritative origin for a version pin; the runtime track is only its fallback
- `templates/blueprint-template.md` — the section contract; read the section count off it in Sweep 0, and missing sections are findings
- `templates/tasks-schema.md` — the emission checklist Sweep 9 applies
- `templates/claude-md-template.md` — the 200-line cap and pre-flight checklist Sweep 8 applies to §19.1
- `knowledge/skills-registry.md` — authoritative skill names, invocation forms, install commands
- `commands/architect-brownfield.md` — the authority on §9.1's required parts; Sweep 0 reads them off that file
- `commands/architect-audit.md` — the command that drives this agent
